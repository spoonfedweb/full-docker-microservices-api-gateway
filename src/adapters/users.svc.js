...
export default class UsersSvcAdapter {
  ...
  static async deleteUserSession({ sessionId }) {
    const body = await got
      .delete(`${USERS_SVC_URL}/sessions/${sessionId}`)
      .json()
  ...
  static async fetchUser({ userId }) {
    try {
      const body = await got.get(`${USERS_SVC_URL}/users/${userId}`).json()
      return body
    } catch (e) {
      return `error: ${e}`
    }
  ...
  static async fetchUserSession({ sessionId }) {
    try {
      const body = await got
        .get(`${USERS_SVC_URL}/sessions/${sessionId}`)
        .json()
      return body
    } catch (e) {
      return `error: ${e}`
